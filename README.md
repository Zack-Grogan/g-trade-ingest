...
- **Endpoints:** `POST /ingest/state`, `POST /ingest/events`, `POST /ingest/trades`, `POST /ingest/account-trades`, `POST /ingest/state-snapshots`, `POST /ingest/market-tape`, `POST /ingest/decision-snapshots`, `POST /ingest/order-lifecycle`, `POST /ingest/run-manifest`, `POST /ingest/bridge-health`, `POST /ingest/runtime-logs`
...
Normalized account identity is promoted into queryable columns for runs, state snapshots, run manifests, completed trades, and account-trade history. This lets downstream analytics and web distinguish practice vs live accounts instead of inferring from raw payload JSON.
